...
# labels = np.load('../model/labelsCorrect.npy')
# word = list(set(list(labels)))
# word.sort()
# label_dict = dict(zip(range(len(word)), word))
...
# model.add(tf.keras.layers.Conv3D(16, (3, 3, 3), activation='relu', input_shape=input_shape, kernel_regularizer=regularizers.l2(0.001)))
...
# model.add(tf.keras.layers.Conv3D(64, (3, 3, 3), activation='relu', kernel_regularizer=regularizers.l2(0.001)))
# model.add(tf.keras.layers.BatchNormalization())
# model.add(tf.keras.layers.MaxPooling3D((2, 2, 2)))
# model.add(tf.keras.layers.Dropout(0.25))
...
# model.load_weights('../model/model_weights1_151.h5', by_name=True)
model.load_weights('../models/model_weights_base.h5',by_name=True)
